...
def mixcr_procedure(path_to_mixcr, fastq_path, outpath, sample, chains, mmu):

    align_cmd, assemble_cmd, exportAlignments_cmd, exportClones_cmds = get_mixcr_cmds(fastq_path, outpath, sample, mmu, chains)
...
def get_mixcr_cmds(fastq_path, outpath, sample, MMU, chains):
    ...
    logger.debug('os.path.join(fastq_path, sample + \'R1.fastq\') ' + os.path.join(fastq_path, sample + 'R1.fastq'))

    fastq1 = os.path.join(fastq_path, sample + 'R1.fastq')
    ...
    assemble_cmd = ('java -Xmx4g -Xms3g -jar mixcr.jar assemble'                    #assemble command
                    ...
                    ' -f'                                                           #overwrite output file if already exists
                    ' -i ' + outpath + '/' + sample + '_index_file'           #keep mapping between initial reads and final clones
    ...
    exportAlignments_cmd = ('java -Xmx4g -Xms3g -jar mixcr.jar exportAlignments'    #exportAlignments command   
                            ...
                            ' -cloneIdWithMappingType ' + outpath + '/' + sample + '_index_file'  #indicate stase of each read
                            ' ' + vdjca_path  +   #input file- VDJCA from previous step
    ...
    for chain in chains:
        exportClones_cmd = ('java -Xmx4g -Xms3g -jar mixcr.jar exportClones'            #exportClones command
                        ...
                        ' -readIds ' + outpath + '/' + sample + '_index_file'
